...
*/
public class PrinterManagementImpl /*implements PrinterManagement*/ {
/*
    ...
    public PrinterManagementImpl() {
        ...
        try {
            ...
            // get a listener
            PrinterEventListener listener = new PrinterEventListener(this);
            ...
            leaseRenewalManager.renewUntil(observerLease, Lease.FOREVER, null);

            // get the scheduling service
            SchedulingService schedulingService = ServiceFinder.getSchedulingService();
            ...
            Schedule turnOffSchedule = schedulingService.newRepeatedDateSchedule(
                    startDate, endDate, monthsOff, null,
                    daysOfWeekOff, hoursOff, minutesOff,
                    calendar.getTimeZone());

            // create the message description
            LocalizableMessage turnOffMessage = new LocalizableMessage(PrinterManagementImpl.class,
                    "TurnOffPrinter", null, null);

            // define the handback object of the
            // turn-off printer task
            MarshalledObject handbackOff = new MarshalledObject(new String("turn-off"));
            ...
            // create the message description
            LocalizableMessage turnOnMessage = new LocalizableMessage(PrinterManagementImpl.class, "TurnOnPrinter", null, null);

            // define the handback object of the
            // turn-on printer task
            MarshalledObject handbackOn = new MarshalledObject(new String("turn-on"));

            // schedule task and get the Ticket for the scheduled task
            turnOnPrinter = schedulingService.scheduleTask(listener, turnOnMessage, turnOnSchedule, SchedulingService.NONE, handbackOn);
        } catch (Exception exception) { // handle exception schedulling task
            System.out.println("PrinterManagementImpl: " + "Exception occurred when scheduling tasks.");
    ...
    public void terminateScheduledTasks() {
        ...
        } catch (Exception exception) { // handle exception canceling scheduled task
            System.out.println("PrinterManagementImpl: " + "Exception occurred when canceling tasks.");
            System.out.println("Please read debug file ...\n");
            Debug.debugException("cancel scheduled task", exception);
        }
    }

    // add paper to printer
    public void addPaper(int amount) {
        System.out.println("PrinterManagementImpl: Adding paper ...\n");
    ...
    public void cancelPendingPrintJobs() {
        System.out.println("PrinterManagementImpl: " + "Canceling pending print jobs ... \n");
        printer.cancelPendingPrintJobs();
    }

    // receive notifications
    public void notify(RemoteEvent remoteEvent) throws UnknownEventException, RemoteException {
        String subString = "com.deitel.advjhtp1.jiro.DynamicService.printer";
    ...
    public void addToner() {
        System.out.println("PrinterManagementImpl: Adding toner ...\n");
        printer.addToner();
    }
    ...
    private void performTask(RemoteEvent remoteEvent) {
        ...
        try {
            // get task type
            String type = (String) remoteEvent.getRegistrationObject().get();
            ...
            if (type.equals("turn-off"))
                ...
                printer.setOnline();
        } catch (Exception exception) {         // handle exception performing scheduled task
            System.out.println("PrinterManagementImpl: " + "Exception occurred when performing tasks.");
            System.out.println("Please read debug file ...\n");
            Debug.debugException("perform scheduled task", exception);
        }
    }

    // handle event
    private synchronized void eventHandler(RemoteEvent remoteEvent) {
        String source = (String) remoteEvent.getSource();
        ...
        LocalizableMessage localizableMessage = new LocalizableMessage(PrinterManagementImpl.class, "Event", params, Locale.US);

        // define log message
        LogMessage logMessage = new LogMessage(localizableMessage, LogMessage.TRACE + ".printer." + source, null);

        // post the log message
        try {
            logService.log(logMessage);
        } catch (Exception exception) { // handle exception posting log message
