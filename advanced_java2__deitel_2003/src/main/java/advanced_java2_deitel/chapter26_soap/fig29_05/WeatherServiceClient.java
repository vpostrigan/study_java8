...
public class WeatherServiceClient extends JFrame {
    ...
    public WeatherServiceClient(String server) {
        ...
        try {
            ...
            URL url = new URL("http://" + server + ":8080/soap/servlet/rpcrouter");

            // build SOAP RPC call
            Call remoteMethod = new Call();
            remoteMethod.setTargetObjectURI("urn:xml-weather-service");
            // set name of remote method to be invoked
            remoteMethod.setMethodName("getWeatherInformation");
    ...
    public List createBeans(Vector weatherStrings) {
        ...
        for (int i = 0; (weatherStrings.size() - 1) > i; i += 3) {
