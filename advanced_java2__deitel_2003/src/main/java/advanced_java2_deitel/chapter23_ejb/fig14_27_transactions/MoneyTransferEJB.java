...
// funds from an Account at BankABC to an Account at BankXYZ using bean-managed transaction demarcation.
public class MoneyTransferEJB implements SessionBean {
    ...
    public void transfer(double amount) throws EJBException {
        ...
        try {
            ...
            // commit transaction
            transaction.commit();
        } catch (Exception exception) { // handle exceptions when withdrawing, depositing and committing transaction

            // attempt rollback of transaction
            try {
                transaction.rollback();
            } catch (SystemException systemException) { // handle exception when rolling back transaction
