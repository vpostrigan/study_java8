...
public class TipTestServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        ...
        try {
            ...
            String jdbcDriver = getServletConfig().getInitParameter("JDBC_DRIVER");

            Class.forName(jdbcDriver); // load JDBC driver

            // get database URL from servlet container
            String databaseUrl = getServletConfig().getInitParameter("DATABASE_URL");
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            ResultSet resultSet = statement.executeQuery("SELECT * FROM tipInfo");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (userAgent.indexOf(ClientUserAgentHeaders.IE) != -1) {
                ...
                Document document = createXMLTipTestAnswer(request);

                // set appropriate Content-Type for client
                response.setContentType("text/html");

                // send XML content to client after XSLT
                applyXSLT("XHTML/XHTMLTipAnswer.xsl", document, response);
            } else if (userAgent.indexOf(ClientUserAgentHeaders.WAP) != -1) { // if WAP client is requesting client
                ...
                // send XML content to client after XSLT
                applyXSLT("WAP/WAPTipAnswer.xsl", document, response);
            } else if (userAgent.indexOf(ClientUserAgentHeaders.IMODE) != -1) { // if i-mode client is requesting client
                ...
                // send XML content to client after XSLT
                applyXSLT("iMode/IMODETipAnswer.xsl", document, response);
            } else if (userAgent.indexOf(ClientUserAgentHeaders.J2ME) != -1) // if J2ME client is requesting client
                sendJ2MEAnswer(request, response);
        } catch (NullPointerException e) { // handle exception if Document is null
            e.printStackTrace();
        }
    }

    // send Tip-Test data to client
    private void sendTipTestQuestion(HttpServletRequest request, HttpServletResponse response, ResultSet resultSet) throws IOException {
        // send ResultSet to appropriate client
        try {
            // determine User-Agent header
            String userAgent = request.getHeader("User-Agent");
            ...
            if (userAgent.indexOf(ClientUserAgentHeaders.IE) != -1) {
                ...
                Document document = createXMLTipTestQuestion(resultSet, request,
                        request.getContextPath() + "/XHTML/images/", ".gif");

                // set appropriate Content-Type for client
                response.setContentType("text/html");
                applyXSLT("XHTML/XHTMLTipQuestion.xsl", document, response);
            } else if (userAgent.indexOf(ClientUserAgentHeaders.WAP) != -1) { // if WAP client is requesting client
                Document document = createXMLTipTestQuestion(resultSet, request,
                        request.getContextPath() + "/WAP/images/", ".wbmp");

                // set appropriate Content-Type for client
                response.setContentType("text/vnd.wap.wml");
                applyXSLT("WAP/WAPTipQuestion.xsl", document, response);
            } else if (userAgent.indexOf(ClientUserAgentHeaders.IMODE) != -1) { // if i-mode client is requesting client

                Document document = createXMLTipTestQuestion(resultSet, request,
                        request.getContextPath() + "/iMode/images/", ".gif");

                // set appropriate Content-Type for client
                response.setContentType("text/html");
                applyXSLT("iMode/IMODETipQuestion.xsl", document, response);
            } else if (userAgent.indexOf(ClientUserAgentHeaders.J2ME) != -1) // if J2ME client is requesting client
                sendJ2MEClientResponse(resultSet, request, response);
        } catch (NullPointerException nullPointerException) { // handle exception if Document is null
            nullPointerException.printStackTrace();
        }
    }

    // send tip test to Internet Explorer client
    private Document createXMLTipTestQuestion(ResultSet resultSet, HttpServletRequest request,
                                              String imagePrefix, String imageSuffix) throws IOException {
        ...
        int correctAnswer = Math.abs(random.nextInt()) % randomRow.length;
        ...
        session.setAttribute("correctAnswer", new Integer(correctAnswer));
        // store correct tip name
        session.setAttribute("correctTipName", new String(resultTable[correctRow][1]));
        // store correct tip description
        session.setAttribute("correctTipDescription", new String(resultTable[correctRow][2]));

        // determine image to send client
        String imageName = imagePrefix + resultTable[correctRow][3] + imageSuffix;
        ...
        try {
            ...
            image.appendChild(document.createTextNode(imageName));
            ...
            for (int i = 0; i < randomRow.length; i++) {
                ...
                choice.appendChild(document.createTextNode(resultTable[randomRow[i]][4]));

                // set choice Element as correct or incorrect
                Attr attribute = document.createAttribute("correct");
            ...
            return document;
        } catch (ParserConfigurationException e) { // handle exception building Document
            e.printStackTrace();
        }
    ...
    private void sendJ2MEClientResponse(ResultSet resultSet,
        ...
        String imageName = "/j2me/images/" + resultTable[correctRow][3] + ".png";
        ...
        try {
            ...
            for (int row = 0; resultSet.next(); row++) {
                ...
                for (int column = 0; column < 5; column++) {
                    ...
                    resultTable[row][column] += resultSet.getObject(column + 1);
                }
            }
        } catch (SQLException sqlException) { // handle exception if servlet cannot get ResultSet Object
            sqlException.printStackTrace();
            return null;
        }
    ...
    private void applyXSLT(String xslFile, Document xmlDocument, HttpServletResponse response) throws IOException {
        // apply XSLT
        try {
        ...
        } catch (TransformerException exception) { // handle exception transforming content
            exception.printStackTrace();
        }
    }

    // create XML Document that stores Tip Test answer
    private Document createXMLTipTestAnswer(HttpServletRequest request) throws IOException {
        // get session
        HttpSession session = request.getSession();

        // match correct answer with session answer
        Integer integer = (Integer) session.getAttribute("correctAnswer");
        int correctAnswer = integer.intValue();

        // give client correct tip name and description
        String correctTipName = (String) session.getAttribute("correctTipName");
        String correctTipDescription = (String) session.getAttribute("correctTipDescription");

        // get user selection
        int selection = Integer.parseInt(request.getParameter("userAnswer"));
        ...
        String servletName = request.getContextPath() + "/" + getServletConfig().getServletName();
        ...
        try {
            ...
            correct.appendChild(document.createTextNode(answer));
            root.appendChild(correct);

            // append Element that describes tip name
            Element name = document.createElement("correctTipName");
            name.appendChild(document.createTextNode(correctTipName));
            root.appendChild(name);

            // append Element that describes tip description
            Element description = document.createElement("correctTipDescription");
            description.appendChild(document.createTextNode(correctTipDescription));
            root.appendChild(description);

            // append Element that links to TipTestServlet
            Element servletLink = document.createElement("servletName");
            servletLink.appendChild(document.createTextNode(servletName));
            root.appendChild(servletLink);

            return document;
        } catch (ParserConfigurationException parserException) { // handle exception building Document
    ...
    private void sendJ2MEAnswer(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    public void destroy() {
        ...
        try {
            connection.close();
        } catch (SQLException sqlException) { // handle if connection cannot be closed
            sqlException.printStackTrace();
        }
    }
