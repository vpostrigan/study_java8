...
public class CloudscapeDataAccess implements AddressBookDataAccess {
    ...
    public CloudscapeDataAccess() throws Exception {
        ...
        sqlFind = connection.prepareStatement(
                "SELECT names.personID, firstName, lastName, " +
                        ...
                        "zipcode, phoneID, phoneNumber, emailID, emailAddress " +
        ...
        sqlPersonID = connection.prepareStatement(
                "VALUES ConnectionInfo.lastAutoincrementValue('APP', 'NAMES', 'PERSONID')");

        // Insert first and last names in table names.
        // For referential integrity, this must be performed
        // before sqlInsertAddress, sqlInsertPhone and sqlInsertEmail.
        ...
        sqlInsertEmail = connection.prepareStatement(
                "INSERT INTO emailAddresses ( personID, emailAddress ) " +
                        "VALUES ( ? , ? )");
        ...
        sqlUpdateAddress = connection.prepareStatement(
                "UPDATE addresses SET address1 = ?, address2 = ?, city = ?, state = ?, zipcode = ? " +
                        "WHERE addressID = ?");
        ...
        // after sqlDeleteAddress, sqlDeletePhone and sqlDeleteEmail,
    ...
    // Obtain a connection to addressbook database.
    // Method may throw ClassNotFoundException or SQLException.
    // If so, exception is passed via this class's constructor back to
    // the AddressBook application so the application can display an error message and terminate.
    ...
    // Locate specified person. Method returns AddressBookEntry containing information.
    ...
    // Update an entry. Method returns boolean indicating success or failure.
    ...
    // Insert new entry. Method returns boolean indicating success or failure.
    ...
    // Delete an entry. Method returns boolean indicating success or failure.
    ...
    // Method to clean up database connection.
    // Provided in case CloudscapeDataAccess object is garbage collected.
    @Override
